...
texto_simples = "Streamlit facilita a criação de aplicações web interativas com Python."
...
st.write("Aqui está um dataframe:", df)  # Exibe o DataFrame com formatação padrão
...
opcao_selectbox = st.selectbox("Selecione uma opção", ["Opção A", "Opção B", "Opção C"])
...
st.write("Opções selecionadas:", opcoes_multiselect)  # Exibe as opções selecionadas
...
st.write("Intervalo selecionado:", intervalo_slider)  # Exibe o intervalo selecionado
